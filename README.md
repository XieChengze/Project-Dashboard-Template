...
There are thousands of data entries in the database generated by AI, and the data includes time. Some query functions in the dashboard can only search for data within the corresponding time range. Therefore, if the dashboard is used too late, some data may expire, resulting in no rows being displayed in the query results.
